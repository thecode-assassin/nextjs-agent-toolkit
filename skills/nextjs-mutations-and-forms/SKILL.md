---
name: nextjs-mutations-and-forms
description: Design, implement, and review Next.js App Router mutations and forms using Server Actions, progressive enhancement, validation, action state, pending UI, redirects, and cache updates. Use for same-application writes, form workflows, mutation ownership, or post-submit freshness.
---

# Next.js Mutations and Forms
...
- Server Action versus HTTP endpoint ownership
- trusted server validation, authorization, action state, pending UI, and progressive enhancement
- redirect, refresh, and cache invalidation after successful writes

## Decision Rules

- Treat every action as a public server entry point: authenticate, authorize, and validate inside it.
- Return serializable expected errors; reserve thrown errors for exceptional failures and framework control flow.
- Choose a single post-write freshness mechanism that matches the visible data owner.
